...
`TODO.md` を作成する際には、下記の原則に従ってください。

- 必ずチェックボックス（例. `- [ ] CLI で 1 つ以上のサイズの引数を受け取り、そのまま表示する機能を実装する（エラー時はステータスコード 1 で終了）。`）で構成すること
- ファイル名やパッケージ名、関数名など、具体的な命名は学習者が決定できるようにすること
- 各タスク完了時にビルドが通るようにし、テストなどで動作確認ができるようにすること
...
完了報告があった場合、必ず差分を確認し、問題がなければ次のステップに進んでください。
